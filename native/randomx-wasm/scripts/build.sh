...
link_flags=(
  -O3 -sMODULARIZE=1 -sEXPORT_NAME=createArcalsRandomX
  ...
  -sEXPORTED_RUNTIME_METHODS=HEAPU8 -sSTACK_SIZE=1048576 -sFILESYSTEM=0
)
all_objects=("${objects[@]}" "$build_dir/arcals_randomx_wasm.o")

# Shipped: one self-contained module Worker (glue plus Worker logic, no
# imports) and the WebAssembly it is given by the page.
rm -rf "$dist_dir"
mkdir -p "$dist_dir" "$build_dir/worker" "$build_dir/test"
em++ "${all_objects[@]}" -o "$build_dir/worker/glue.js" "${link_flags[@]}" \
  -sENVIRONMENT=worker
{
  echo "// Arcals browser RandomX Worker. Generated by native/randomx-wasm/scripts/build.sh"
  echo "// from the pinned RandomX source with the Arcals patches; see the README."
  cat "$build_dir/worker/glue.js"
  cat "$wasm_root/js/browser-worker.js"
} > "$dist_dir/randomx-browser-worker.mjs"
cp "$build_dir/worker/glue.wasm" "$dist_dir/randomx-browser.wasm"

# Test only: an ES module build that Node can import directly.
em++ "${all_objects[@]}" -o "$build_dir/test/arcals-randomx.mjs" "${link_flags[@]}" \
  -sEXPORT_ES6=1 -sENVIRONMENT=web,worker,node

( cd "$dist_dir" && sha256sum randomx-browser-worker.mjs randomx-browser.wasm > SHA256SUMS && cat SHA256SUMS )
